...
# Get next OTS
OTS_KEY=$(qrl-cli ots $QRL_ADDRESS -m -j |grep next_key |jq .[0].next_key)
...
NOTARIZE=$(qrl-cli notarize $SHASUM -m -M "https://QRL.CO.IN Chain State" -w $QRL_DIR/$QRL_WALLET -i $OTS_KEY -j )
